...
详细的版本进入条件和暂缓方向记录在[版本路线](ROADMAP.zh-CN.md)中。当前 `0.2.0-alpha.4` 方向会继续把 `doctor` 做成更强的仓库启动医生：adapter、启动计划、probe、日志分类、评分回归、corpus promotion、历史 scorecard 和下一步行动会优先于单纯的速度承诺。
...
- failure dataset history：每次 review 可以追加 scorecard 快照，并和上一轮扫描对比 improved、regressed、unchanged 和 not comparable 信号
