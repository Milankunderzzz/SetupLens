...
test('failure corpus cases reproduce expected doctor diagnoses', async () => {
  const { results, metrics, passed } = await evaluateCorpus({ timeoutMs: 3000 });
  ...
  assert.equal(metrics.diagnosticHitRate, 100);
  assert.equal(metrics.safeFixGenerationRate, 100);
  assert.equal(metrics.falseBlockerCount, 0);
  assert.ok(metrics.rootCauseFirstRate >= 70);
